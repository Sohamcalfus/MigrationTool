...
def transform_date_column(df, column_name, date_format='%Y-%m-%d'):
    """Transform a date column to the specified format, handling errors gracefully."""
    if column_name in df.columns:
        ...
        print(f"✓ Date transformed: {column_name} to format {date_format}")
    return df

def transform_numeric_column(df, column_name, decimal_places=2):
    """Convert a column to numeric, coercing errors to NaN."""
    if column_name in df.columns:
        # Convert to numeric and round to specified decimal places
        df[column_name] = pd.to_numeric(df[column_name], errors='coerce').round(decimal_places)
        print(f"✓ Numeric transformed: {column_name} to {decimal_places} decimal places")
    return df

def transform_text_column(df, column_name, max_length=None, uppercase=False):
    """Transform text column with length limits and case conversion."""
    if column_name in df.columns:
        ...
        print(f"✓ Text transformed: {column_name} (uppercase={uppercase}, max_length={max_length})")
    return df

def apply_oracle_transformations(df):
    """Apply Oracle Fusion specific data transformations"""
    print("\nApplying Oracle Fusion data transformations...")
    
    # Date transformations - Oracle expects YYYY-MM-DD format
    date_columns = {
        'TRX_DATE': '%Y-%m-%d',
        'GL_DATE': '%Y-%m-%d', 
        'CONVERSION_DATE': '%Y-%m-%d',
        'Transaction Date': '%Y-%m-%d',
        'Accounting Date': '%Y-%m-%d'
    }
    
    for col, date_fmt in date_columns.items():
        df = transform_date_column(df, col, date_fmt)
    
    # Numeric transformations - Oracle expects specific precision
    numeric_columns = {
        'AMOUNT': 2,
        'UNIT_SELLING_PRICE': 4,
        'QUANTITY_INVOICED': 0,
        'CONVERSION_RATE': 6,
        'Transaction Line Amount': 2,
        'Unit Selling Price': 4,
        'Transaction Line Quantity': 0
    }
    
    for col, decimals in numeric_columns.items():
        df = transform_numeric_column(df, col, decimals)
    
    # Text transformations - Oracle has specific requirements
    text_columns = {
        'CURRENCY_CODE': {'max_length': 15, 'uppercase': True},
        'BATCH_SOURCE_NAME': {'max_length': 50, 'uppercase': False},
        'DESCRIPTION': {'max_length': 240, 'uppercase': False},
        'TRX_NUMBER': {'max_length': 20, 'uppercase': False},
        'CUST_TRX_TYPE_NAME': {'max_length': 20, 'uppercase': False},
        'Transaction Line Type': {'max_length': 20, 'uppercase': True},
        'ORIG_SYSTEM_BILL_CUSTOMER_REF': {'max_length': 240, 'uppercase': False},
        'ORIG_SYSTEM_BILL_ADDRESS_REF': {'max_length': 240, 'uppercase': False}
    }
    
    for col, params in text_columns.items():
        df = transform_text_column(df, col, params['max_length'], params['uppercase'])
    
    # Special transformations for Oracle requirements
    
    # Ensure LINE_NUMBER is integer
    if 'LINE_NUMBER' in df.columns:
        df['LINE_NUMBER'] = df['LINE_NUMBER'].astype(int)
        print("✓ LINE_NUMBER converted to integer")
    
    # Ensure boolean flags are Y/N
    boolean_columns = ['TAX_EXEMPT_FLAG']
    for col in boolean_columns:
        if col in df.columns:
            df[col] = df[col].map({'True': 'Y', 'False': 'N', True: 'Y', False: 'N', 1: 'Y', 0: 'N'}).fillna('N')
            print(f"✓ Boolean transformed: {col} to Y/N format")
    
    # Remove any null values that Oracle can't handle
    print("✓ Cleaning null values for Oracle compatibility...")
...
def process_fbdi_receivables():
    """Complete FBDI processing for Oracle Fusion Receivables with data transformations"""
    ...
    # NEW Step 10: Apply Oracle Fusion data transformations
    print("\nStep 10: Applying Oracle Fusion data transformations...")
    final_df = apply_oracle_transformations(final_df)
    
    # Step 11: Final verification and CSV generation
    ...
    print(f"✅ Oracle data transformations applied")
...
- Oracle Fusion data transformations applied:
  * Dates formatted as YYYY-MM-DD
  * Numeric fields rounded to appropriate decimal places
  * Text fields truncated to Oracle limits
  * Currency codes converted to uppercase
  * Boolean flags converted to Y/N format
...
if __name__ == "__main__":
    try:
        ...
        if zip_file and csv_file:
            ...
            print(f"📁 Data transformations applied + duplicates eliminated!")
