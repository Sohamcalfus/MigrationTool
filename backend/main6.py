...
def generate_fbdi_from_table():
    ...
    try:
        ...
        for col_idx, template_col in enumerate(template_columns):
            ...
            # Skip *Buisness Unit Name column entirely
            if template_col == "*Buisness Unit Name":
                print(f"✓ Skipped '*Buisness Unit Name' column as requested")
                continue
            
            # Special case: Comments (template) -> *Business Unit Name (raw)
            if template_col == "Comments" and has_buisness_unit_in_raw:
                ...
                print(f"✓ Special mapping: {template_col} <- *Buisness Unit Name")
                continue
        ...
        # Remove '*Buisness Unit Name' column if present before saving CSV
        if '*Buisness Unit Name' in template_columns:
            ...
            print(f"✓ Removed '*Buisness Unit Name' column from output")

        # Remove header rows (keep only data rows)
        data_only_df = template_df.iloc[start_row:start_row + num_rows]
        print(f"✓ Removed header rows - keeping only data rows")
...
def generate_fbdi():
    ...
    try:
        ...
        for col_idx, template_col in enumerate(template_columns):
            ...
            # Skip *Buisness Unit Name column entirely - no mapping or database storage
            if template_col == "*Buisness Unit Name":
                print(f"✓ Skipped '*Buisness Unit Name' column as requested - no mapping created")
                continue
            
            # Handle special case: Comments (template) -> *Business Unit Name (raw)
            if template_col == "Comments" and has_buisness_unit_in_raw:
                raw_col_idx = raw_columns.index("*Buisness Unit Name")
                template_df.iloc[start_row:start_row + num_rows, col_idx] = raw_data.iloc[:, raw_col_idx].values
                raw_col_name = "*Buisness Unit Name"
                status = 'Y'
                print(f"✓ Special mapping: {template_col} <- {raw_col_name}")
            
            # Handle regular direct mappings (column names match exactly)
            elif template_col in raw_columns:
            ...
            # Save mapping to database (excluding *Buisness Unit Name)
...
def generate_mappings_only():
    """Generate and store mappings without creating download file"""
    try:
        # Receive uploaded files
        template_file = request.files['template_file']
        raw_file = request.files['raw_file']
